...
public class RevenantEntity extends MonsterEntity implements IChargeableMob, IRangedAttackMob {
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(2,
				new NearestAttackableTargetGoal<>(this, MobEntity.class, 0, false, false, NOT_UNDEAD));
	}
	...
	public void livingTick() {
		...
		if (!this.world.isRemote && this.getWatchedTargetId(0) > 0) {
			...
			if (entity != null) {
				double d0 = vec3d.y;
				if (this.getPosY() < entity.getPosY()
						|| !this.isCharged() && this.getPosY() < entity.getPosY() + 5.0D) {
					d0 = Math.max(0.0D, d0);
		...
		for (int l = 0; l < 3; ++l) {
			...
			this.world.addParticle(ParticleTypes.SMOKE, d8 + this.rand.nextGaussian() * (double) 0.3F,
					d10 + this.rand.nextGaussian() * (double) 0.3F, d2 + this.rand.nextGaussian() * (double) 0.3F, 0.0D,
					0.0D, 0.0D);
			if (flag && this.world.rand.nextInt(4) == 0) {
				this.world.addParticle(ParticleTypes.ENTITY_EFFECT, d8 + this.rand.nextGaussian() * (double) 0.3F,
						d10 + this.rand.nextGaussian() * (double) 0.3F, d2 + this.rand.nextGaussian() * (double) 0.3F,
						(double) 0.7F, (double) 0.7F, 0.5D);
			}
		}

		if (this.getInvulTime() > 0) {
			for (int i1 = 0; i1 < 3; ++i1) {
				this.world.addParticle(ParticleTypes.ENTITY_EFFECT, this.getPosX() + this.rand.nextGaussian(),
						this.getPosY() + (double) (this.rand.nextFloat() * 3.3F),
						this.getPosZ() + this.rand.nextGaussian(), (double) 0.7F, (double) 0.7F, (double) 0.9F);
			}
		}
	...
	protected void updateAITasks() {
		if (this.getInvulTime() > 0) {
			...
			if (j1 <= 0) {
				Explosion.Mode explosion$mode = net.minecraftforge.event.ForgeEventFactory
						.getMobGriefingEvent(this.world, this) ? Explosion.Mode.DESTROY : Explosion.Mode.NONE;
				this.world.createExplosion(this, this.getPosX(), this.getPosYEye(), this.getPosZ(), 7.0F, false,
						explosion$mode);
				...
				if (this.ticksExisted >= this.nextHeadUpdate[i - 1]) {
					...
					if (this.world.getDifficulty() == Difficulty.NORMAL
						...
						if (k3 > 15) {
							float f = 10.0F;
							float f1 = 5.0F;
							double d0 = MathHelper.nextDouble(this.rand, this.getPosX() - 10.0D,
									this.getPosX() + 10.0D);
							double d1 = MathHelper.nextDouble(this.rand, this.getPosY() - 5.0D, this.getPosY() + 5.0D);
							double d2 = MathHelper.nextDouble(this.rand, this.getPosZ() - 10.0D,
									this.getPosZ() + 10.0D);
					...
					if (k1 > 0) {
						...
						if (entity != null && entity.isAlive() && !(this.getDistanceSq(entity) > 900.0D)
						...
						List<LivingEntity> list = this.world.getTargettableEntitiesWithinAABB(LivingEntity.class,
								field_213798_bB, this, this.getBoundingBox().grow(20.0D, 8.0D, 20.0D));
					...
					for (int k2 = -1; k2 <= 1; ++k2) {
						for (int l2 = -1; l2 <= 1; ++l2) {
							for (int j = 0; j <= 3; ++j) {
								...
								if (blockstate.canEntityDestroy(this.world, blockpos, this)
	...
	private void launchWitherSkullToEntity(int p_82216_1_, LivingEntity p_82216_2_) {
		this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.getPosX(),
				p_82216_2_.getPosY() + (double) p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.getPosZ(),
		...
		if (this.isInvulnerableTo(source)) {
			...
			if (this.getInvulTime() > 0 && source != DamageSource.OUT_OF_WORLD) {
				...
				if (entity1 != null && !(entity1 instanceof PlayerEntity) && entity1 instanceof LivingEntity
						&& ((LivingEntity) entity1).getCreatureAttribute() == this.getCreatureAttribute()) {
					return false;
				} else {
					if (this.blockBreakCounter <= 0) {
